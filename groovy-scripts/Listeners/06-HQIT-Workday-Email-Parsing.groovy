...
def orientationRegionField = customFieldManager.getCustomFieldObjects(issue).find {it.name == "Orientation Region"}
def idField = customFieldManager.getCustomFieldObjects(issue).find {it.name == "ID"}
...
if (issue.issueTypeObject.name == "Master Data") {
    ...
    def id = tokenizeDescription[13].trim()
    def workcity = tokenizeDescription[15].trim()
    def workstate = tokenizeDescription[17].trim()
    def country = tokenizeDescription[19].trim()
    //def region = tokenizeDescription[17].trim()
    def location = tokenizeDescription[21].trim()
    //def linebreak = tokenizeDescription[20]
    def orientation = tokenizeDescription[23].trim()
    def manager = tokenizeDescription[25].trim()
	def recruiter = tokenizeDescription[27].trim()
   // def device = tokenizeDescription[29].trim().replace('[','').replace(']','')
    ...
    deviceField.updateValue(null, issue, new ModifiedValue(issue.getCustomFieldValue(deviceField), ''), changeHolder);
    ...
    orientationRegionField.updateValue(null, issue, new ModifiedValue(issue.getCustomFieldValue(orientationRegionField), region), changeHolder);
    idField.updateValue(null, issue, new ModifiedValue(issue.getCustomFieldValue(idField), id), changeHolder);
